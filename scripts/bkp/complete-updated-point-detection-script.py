...
def calculate_player_features(df):
    df['n_hs'] = df['n_kp'].apply(lambda x: np.array(json.loads(x))[9:11][:,:2] if isinstance(x, str) else np.array([]))
    df['f_hs'] = df['f_kp'].apply(lambda x: np.array(json.loads(x))[9:11][:,:2] if isinstance(x, str) else np.array([]))
    df['n_ls'] = df['n_kp'].apply(lambda x: np.array(json.loads(x))[15:17][:,:2] if isinstance(x, str) else np.array([]))
    df['f_ls'] = df['f_kp'].apply(lambda x: np.array(json.loads(x))[15:17][:,:2] if isinstance(x, str) else np.array([]))
    df['n_bs'] = df['n_kp'].apply(lambda x: np.array(json.loads(x))[:4][:,:2] if isinstance(x, str) else np.array([]))
    df['f_bs'] = df['f_kp'].apply(lambda x: np.array(json.loads(x))[:4][:,:2] if isinstance(x, str) else np.array([]))
    ...
    df['n_l'] = df['n_kp'].apply(lambda x: np.array(json.loads(x))[15:17][:,:2].mean(axis=0) if isinstance(x, str) else np.array([]))
    df['f_l'] = df['f_kp'].apply(lambda x: np.array(json.loads(x))[15:17][:,:2].mean(axis=0) if isinstance(x, str) else np.array([]))
