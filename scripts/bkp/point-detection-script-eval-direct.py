...
def process_player_data(df):
    ...
    for idx, (f, row) in enumerate(df.iterrows()):
        ...
        if player_boxes.shape[0] == 0 or max([box_area(box) for box in player_boxes]) > 20000: 
            continue 
        court_center = np.array(eval(row.kp))[12:14].mean(axis=0)
...
def calculate_player_features(df):
    df['n_hs'] = df['n_kp'].apply(lambda x: np.array(eval(x))[9:11][:,:2] if isinstance(x, str) else np.array([]))
    df['f_hs'] = df['f_kp'].apply(lambda x: np.array(eval(x))[9:11][:,:2] if isinstance(x, str) else np.array([]))
    df['n_ls'] = df['n_kp'].apply(lambda x: np.array(eval(x))[15:17][:,:2] if isinstance(x, str) else np.array([]))
    df['f_ls'] = df['f_kp'].apply(lambda x: np.array(eval(x))[15:17][:,:2] if isinstance(x, str) else np.array([]))
    df['n_bs'] = df['n_kp'].apply(lambda x: np.array(eval(x))[:4][:,:2] if isinstance(x, str) else np.array([]))
    df['f_bs'] = df['f_kp'].apply(lambda x: np.array(eval(x))[:4][:,:2] if isinstance(x, str) else np.array([]))
    ...
    df['n_l'] = df['n_kp'].apply(lambda x: np.array(eval(x))[15:17][:,:2].mean(axis=0) if isinstance(x, str) else np.array([]))
    df['f_l'] = df['f_kp'].apply(lambda x: np.array(eval(x))[15:17][:,:2].mean(axis=0) if isinstance(x, str) else np.array([]))
...
def main(input_file, output_file):
    ...
    array_columns = ['kp', 'n_kp', 'f_kp', 'n_hs', 'f_hs', 'n_ls', 'f_ls', 'n_bs', 'f_bs', 'n_l', 'f_l']
