...
def download_video(url, start_time, end_time, output_file):
    ...
    subclip_video(video_path, start_time, end_time, str(output_file))

def subclip_video(input_file, start_time, end_time, output_file):
    """
    Subclip the video using FFmpeg.
    """
    ffmpeg_cmd = [
        'ffmpeg',
        '-i', input_file,
        '-ss', start_time,
        '-to', end_time,
        '-c', 'copy',  # This copies the codecs without re-encoding, which is faster
        output_file
    ]
    ...
    print(f"Subclipped video saved to: {output_file}")
...
def extract_frames(input_file, output_folder):
    ...
    ffmpeg_cmd = [
        'ffmpeg',
        '-i', str(input_file),
        '-vf', 'fps=30',  # Extract 30 frames per second, adjust as needed
